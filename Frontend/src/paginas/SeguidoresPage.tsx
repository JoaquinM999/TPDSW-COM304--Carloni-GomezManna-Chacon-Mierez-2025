...
const UserAvatar: React.FC<{ usuario: { id?: number; nombre?: string; username: string; avatar?: string }; size?: string; clickable?: boolean; currentUserId?: number | null; }> = ({ usuario, size = 'w-14 h-14', clickable = true, currentUserId = null }) => {
  const getAvatarSrc = () => {
...
const SeguidoresPage: React.FC = () => {
  ...
  const [perfilTarget, setPerfilTarget] = useState<{ nombre?: string; username?: string } | null>(null);
  ...
  const cargarSeguidores = async () => {
    try {
      ...
      if (!token) {
        setError('Debes iniciar sesión para ver los seguidores.');
        setRequiresAuth(true);
        setLoading(false);
        return;
      }

      const [followers, perfil] = await Promise.all([
        seguimientoService.getSeguidores(userId),
        usuarioService.getPublicProfile(userId).catch(() => null),
      ]);

      setSeguidores(followers);
      setPerfilTarget(perfil);
  ...
  const usuariosFiltrados = seguidores.filter((usuario) =>
  ...
  const headerName = perfilTarget?.nombre || perfilTarget?.username || 'Usuario';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-600 dark:text-gray-300">
            {seguidores.length} {seguidores.length === 1 ? 'usuario' : 'usuarios'} siguen a {headerName}
          </p>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
            <div className="bg-white dark:bg-[#1a2332] rounded-lg p-12 border border-gray-200 dark:border-gray-600 shadow-sm">
              ...
              <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-2">
                {filtro ? 'No se encontraron usuarios' : 'Aún no hay seguidores'}
              </h3>
              <p className="text-gray-600 dark:text-gray-300 mb-6">
                {filtro ? 'Intenta con otro término de búsqueda' : 'Comparte tu perfil para obtener seguidores'}
              </p>
            </div>
          </motion.div>
        ) : (
